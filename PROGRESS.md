...
## ✅ 내일 할일 체크리스트 (2026-06-20)

### 🔴 최우선 (서비스 핵심) — 토요일

- [ ] **사망진단서 OCR 연동** (`/apply/service-info` 고인정보 입력 단계)
  - CLOVA OCR API 이미 env 설정됨, UI만 연결하면 됨
  - 사망진단서 업로드 → 고인 이름 / 사망일 자동 추출 → 입력폼 자동 채우기
  - 실패 시 수동 입력 fallback

- [ ] **위임장 PDF 어드민 다운로드 버그 수정**
  - 현재 어드민에서 다운로드 클릭 시 오류
  - 위임장 내용 법적 요건에 맞게 구체화 (위임 범위 항목 명시)

- [ ] **카카오 알림톡 실 발송 테스트**
  - PFID + 템플릿 ID 오늘 전부 Vercel 등록 완료 → 실제 수신 확인
  - 결제완료 → payment + submitted 동시 수신
  - 환불 → refund 수신
  - 어드민 상태변경 → processing / completed 수신

### 🟠 어드민 개선 — 토요일

- [ ] **구글 시트 연동 점검**
  - 신청 접수 시 자동 행 추가 (현재 상태변경 시 업데이트만 있음)
  - 결제금액 컬럼 기록 확인
- [ ] **어드민 UX 개선**
  - 불필요한 AI 센터 섹션 제거
  - 케이스 목록 필터/정렬 개선
...
### 🟢 전체 플로우 검증 — 일요일 (월요일 검증 전)

- [ ] **신청 전체 플로우 end-to-end 테스트**
  - 약관동의 → 고인정보(OCR) → 서비스선택 → 서류업로드 → 결제 → 완료
  - 카톡 알림 수신 확인
- [ ] **환불 플로우**: 취소하기 → 환불처리 → 카톡 수신 → 결제내역 환불 표시
- [ ] 모바일 실기기 전체 탭 이동 최종 확인
- [ ] 사용자 관점 처음부터 전체 앱 순회
