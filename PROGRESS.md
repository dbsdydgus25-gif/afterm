...
### [개발팀] apply 페이지 UX 개선 4종 (commit e3ab20d, 배포 완료)
- 고인 전화번호 라벨 빨간색 + "필수 입력" 표시 + 카카오·구글 계정 처리 시 반드시 필요 안내 문구 추가
- 유입 경로 단계(referral innerStep) 완전 제거 → 신청인 정보 4단계로 축소
- StepAccountNotice: 아이폰(설정>암호) / 갤럭시(설정>생체인식>비밀번호관리자) 구체적 안내로 개선
- 계정 삭제 트랙에 카카오톡 추가 (TRACK_PLATFORMS delete에 kakaotalk 추가)

### [개발팀] 위임장·탈퇴요청서 PDF 폰트 이중 embed 버그 수정 (commit 6fab4f0, 배포 완료)
- 위임장 PDF, 유저용 위임장 PDF, 카카오 탈퇴 요청서 → Buffer.from() 래핑으로 글자 깨짐 현상 해결

### [개발팀] 위임장 Word(.docx) 자동 작성 기능 신규 추가 (commit 6ab5059, 배포 완료, 테스트 예정)
- `/api/admin/cases/[caseId]/delegation-docx` 신규 API 생성
- 기존 `위임장_에프텀.docx` 템플릿에 케이스 데이터 자동 입력 (jszip 활용)
- 어드민 PackageActions: "위임장 (Word 자동 작성)" 버튼으로 교체

### [개발팀] 어드민 케이스 목록 신청인 이름 표시 수정 (commit 6ab5059, 배포 완료, 테스트 예정)
- delegations join 방식 → 별도 쿼리 방식으로 변경 (RLS join 문제 해결)

### [개발팀] 카카오 탈퇴 요청서 날짜·서명 좌표 수정 (commit 6ab5059, 배포 완료, 테스트 예정)
- 년/월/일 각 글자 앞에 숫자가 올바르게 배치되도록 좌표 수정
- "이   름" 라벨 위에 흰 사각형 덮고 신청인 이름 기입

### [전략기획팀] 플랫폼별 고인 계정 문의처 조사 완료 (어제 완료 반영)
- 카카오/구글/인스타/페북/트위터(X) 5개 플랫폼 정책 파악 완료
- 구글·인스타·페북·트위터: 위임장 기반 제3자 대행 정책적으로 허용 (명문화 확인)
- 카카오: 정책 불명확 → 고객센터 서면 문의 필요

### [마케팅팀] 온라인 채널 전략 수립 완료 (어제 완료 반영)
- 채널 구축 순서 확정: 네이버 블로그 → 인스타 → 카카오 오픈채널 → 유튜브
- 30일 콘텐츠 계획, SEO 키워드, 유튜브 영상 주제 5개 기획 완료

---

## 📋 2026-06-25 내일 할일 체크리스트

### 🔴 최우선 — 대표님 직접 처리 (서비스 동작 직결)

- [ ] **[Supabase] delegator_address 컬럼 추가 SQL 실행** — `ALTER TABLE public.delegations ADD COLUMN IF NOT EXISTS delegator_address text;` 직접 실행 후 테스트 신청으로 주소 저장 여부 확인
- [ ] **[카카오] 위임장 기반 제3자 대행 가능 여부 고객센터 서면 문의** — 서비스 안내 문구 확정 전 반드시 선행 필요
- [ ] **[포트원] KG이니시스 실결제 MID 심사 현황 확인**
- [ ] **[포트원] 카카오페이 가맹점 심사 현황 확인**

### 🔴 최우선 — 개발팀 테스트 및 기능 완성

- [ ] **어드민 위임장 Word 자동 작성 기능 테스트** — commit 6ab5059 배포 후 첫 테스트 (어드민에서 케이스 선택 → Word 다운로드 정상 여부 확인)
- [ ] **어드민 케이스 목록 신청인 이름 표시 확인** — commit 6ab5059 적용 후 목록 정상 노출 여부 확인
- [ ] **카카오 탈퇴 요청서 날짜·서명 좌표 출력 확인** — commit 6ab5059 적용 후 PDF 출력물 육안 검수
- [ ] **네이버 주소 API 연동** — 신청인 주소 입력에 주소 검색 팝업 추가
- [ ] **위임장 PDF 주소 자동 기입** — delegator_address → PDF 위임인 주소 칸 반영
- [ ] **AFTERM001 코드로 전체 플로우 end-to-end 테스트**
...
- [ ] **위임장 PDF 주소 빈칸** — delegator_address 컬럼 SQL 실행 대기 중 (대표님 Supabase 직접 실행 필요)
- [ ] **어드민 위임장 Word / 케이스 목록 / 카카오 탈퇴 요청서 좌표** — commit 6ab5059 배포 완료, 내일 아침 테스트 예정 (미확인)
